...
| GUI server stays running without `--once`. | Low | Expected behavior, easy to mistake for a hang. | Run `node bin/aoc.mjs gui --repo . --run-id smoke`. | Use `--once` when a static HTML snapshot is wanted. |
