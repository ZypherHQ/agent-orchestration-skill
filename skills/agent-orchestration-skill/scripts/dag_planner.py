...
def build_plan(task: str, surfaces: list[str], size: str, risk: str, ambiguity: str, requires_browser: bool, requires_docs: bool, needs_architecture: bool) -> dict:
    ...
    deps = [phases[-1]["id"]]
    verification_id = f"P{len(phases)+1}"
    phases.append(phase(verification_id, "Run verification commands matched to touched surfaces; collect concise evidence and failures.", "verification_engine_medium" if size not in {"XS", "S"} else "test_runner_low", deps=deps, kind="verification", reasoning="medium" if size not in {"XS", "S"} else "low"))

    review_deps = [verification_id]
    if requires_browser:
        browser_id = f"P{len(phases)+1}"
        phases.append(phase(browser_id, "Validate changed UI/user flows with browser QA and capture concrete evidence.", "browser_qa_medium", deps=deps, kind="browser_qa", reasoning="medium"))
        review_deps.append(browser_id)

    if high_risk:
        phases.append(phase(f"P{len(phases)+1}", "Review the final diff for security/regression risk; do not edit files.", "security_reviewer_high", deps=review_deps, kind="review", reasoning="high"))
    elif size in {"L", "XL"}:
        phases.append(phase(f"P{len(phases)+1}", "Review the final diff for regressions and missing tests; do not edit files.", "regression_reviewer_medium", deps=review_deps, kind="review", reasoning="medium"))
