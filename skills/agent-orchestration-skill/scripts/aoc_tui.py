...
def load_sessions(root: Path) -> list[dict[str, Any]]:
    ...
    rows = list((idx.get("runs") or {}).values()) if isinstance(idx, dict) else []
    if not rows:
...
def load_state(root: Path, run_id: str | None) -> dict[str, Any]:
    if not run_id:
        return {}
    return read_json(state_path(root, run_id), {})
...
def lines_sessions(root: Path, sessions: list[dict[str, Any]], selected: int) -> list[str]:
    ...
    if not sessions:
        return out + ["No sessions yet. Invoke $agent-orchestration-skill and initialize a run ledger."]
    for i, r in enumerate(sessions):
        marker = ">" if i == selected else " "
        out.append(f"{marker} {short(r.get('run_id'), 34):34} {short(r.get('status'), 11):11} W:{r.get('worker_count',0):<2} T:{r.get('test_event_count',0):<2} {short(r.get('task'), 60)}")
    return out
...
def lines_usage(root: Path, run_id: str | None) -> list[str]:
    ...
    out.append("")
    out.append("Commands:")
    out.append("  usage_ledger.py report --scope-run --derive --run-id latest")
    out.append("  ccusage_bridge.py run --source codex --report session")
    out.append("")
